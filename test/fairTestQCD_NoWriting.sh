...
# (time shasum -a 1 $original) 2> timesha1.txt 1>/dev/null
...
# echo "test"
# shasum1=$(shasum -a 1 $original | awk '{print $1}' ) 
# echo "test2"

# echo $shasum1 >> sha1.txt

#sha256 original
( time sort -k 1 -t , $original | shasum -a 256 ) 2> timesha256.txt
time="timesha256.txt"
sha256real=$(sed -n 3p $time )
sha256user=$(sed -n 4p $time )
sha256sys=$(sed -n 5p $time )
shasum256=$(shasum -a 256 $original  | awk '{print $1}')
echo $shasum256 >> sha256.txt

#md5 original
( time sort -k 1 -t , $original | md5 ) 2> timemd5.txt
time="timemd5.txt"
...
do
  ...
  # #sha1 sorted
  # ( time sort -k 1 -t , $original | shasum -a 1 ) 2> timesha1.txt
  ...
  sha256user=$(sed -n 4p $time )
  sha256sys=$(sed -n 5p $time )
  shasum256Check=$(shasum -a 256 new_data.csv | awk '{print $1}')
...
#qcd randomized
  ...
  # sha1user=$(sed -n 4p $time )
  # sha1sys=$(sed -n 5p $time )
  # shasum1Check=$(shasum -a 1 new_data.csv | awk '{print $1}')
  # if [[ $shasum1Check == $shasum1 ]]
  # then
  #   sha1="CHECKSUM OK"
  # else
